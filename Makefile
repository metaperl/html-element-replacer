# This Makefile is for the HTML::Element::Replacer extension to perl.
#
...
#     ABSTRACT_FROM => q[lib/HTML/Element/Replacer.pm]
#     AUTHOR => q[Terrence Brannon <tbone@cpan.org>]
#     LICENSE => q[perl]
#     NAME => q[HTML::Element::Replacer]
#     PL_FILES => {  }
#     PREREQ_PM => { Test::More=>q[0], HTML::Element::Library=>q[0], Moose=>q[0.72] }
#     VERSION_FROM => q[lib/HTML/Element/Replacer.pm]
#     clean => { FILES=>q[HTML-Element-Replacer-*] }
...
NAME_SYM = HTML_Element_Replacer
VERSION = 0.05
...
FULLPERL = /usr/bin/perl5.10.0.exe
ABSPERL = $(PERL)
PERLRUN = $(PERL)
...
MAKE = make
FULLEXT = HTML/Element/Replacer
BASEEXT = Replacer
PARENT_NAME = HTML::Element
DLBASE = $(BASEEXT)
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = lib/HTML/Element/Replacer.pod

# Where is the Config information that we are using/depend on
CONFIGDEP = $(PERL_ARCHLIB)$(DFSEP)Config.pm $(PERL_INC)$(DFSEP)config.h

# Where to build things
INST_LIBDIR      = $(INST_LIB)/HTML/Element
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/HTML/Element
...
TO_INST_PM = lib/HTML/Element/Replacer.pm \
	lib/HTML/Element/Replacer.pod

PM_TO_BLIB = lib/HTML/Element/Replacer.pod \
	blib/lib/HTML/Element/Replacer.pod \
...
DISTVNAME = HTML-Element-Replacer-0.05
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW) \
	  lib/HTML/Element/Replacer.pod $(INST_MAN3DIR)/HTML.Element.Replacer.$(MAN3EXT) 
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'name:                HTML-Element-Replacer' >> META_new.yml
	$(NOECHO) $(ECHO) 'version:             0.05' >> META_new.yml
	$(NOECHO) $(ECHO) 'abstract:            ~' >> META_new.yml
	$(NOECHO) $(ECHO) 'license:             perl' >> META_new.yml
	$(NOECHO) $(ECHO) 'author:              ' >> META_new.yml
	$(NOECHO) $(ECHO) '    - Terrence Brannon <tbone@cpan.org>' >> META_new.yml
	$(NOECHO) $(ECHO) 'generated_by:        ExtUtils::MakeMaker version 6.44' >> META_new.yml
	$(NOECHO) $(ECHO) 'distribution_type:   module' >> META_new.yml
	$(NOECHO) $(ECHO) 'requires:     ' >> META_new.yml
	$(NOECHO) $(ECHO) '    HTML::Element::Library:        0' >> META_new.yml
	$(NOECHO) $(ECHO) '    Moose:                         0.72' >> META_new.yml
...
ppd :
	...
	$(NOECHO) $(ECHO) '    <AUTHOR>Terrence Brannon &lt;tbone@cpan.org&gt;</AUTHOR>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="HTML-Element-Library" VERSION="0,0,0,0" />' >> $(DISTNAME).ppd
...
pm_to_blib : $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', '\''$(PM_FILTER)'\'')' -- \
	  lib/HTML/Element/Replacer.pod blib/lib/HTML/Element/Replacer.pod \
	  lib/HTML/Element/Replacer.pm blib/lib/HTML/Element/Replacer.pm 
